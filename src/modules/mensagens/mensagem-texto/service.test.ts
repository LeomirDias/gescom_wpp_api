...
test("MensagensService publica payload com tenantId e metaPhoneNumberId", async () => {
  ensureEnvForTests();
  const { MensagensService } = require("./service") as typeof import("./service");

  let publishedPayload: SendTextMessageJobPayload | null = null;
  const service = new MensagensService(async (payload: SendTextMessageJobPayload) => {
    publishedPayload = payload;
    return {};
  });

  await service.enfileirarMensagemTexto(
    "req-12345678",
    {
      to: "+5511999999999",
      message: "Ola",
  ...
  const payload = publishedPayload as SendTextMessageJobPayload;
